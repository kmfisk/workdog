...
public class JackRussellTerrierEquipmentLayer extends RenderLayer<JackRussellTerrierEntity, JackRussellTerrierModel> {
    private final JackRussellTerrierModel model;

    public JackRussellTerrierEquipmentLayer(RenderLayerParent<JackRussellTerrierEntity, JackRussellTerrierModel> layerParent, EntityModelSet modelSet) {
        super(layerParent);
        model = new JackRussellTerrierModel.Adult(modelSet.bakeLayer(JackRussellTerrierModel.EQUIPMENT_LAYER));
    }

    @Override
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, JackRussellTerrierEntity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        for (AbstractInventoryAnimal.DogEquipmentType dogEquipmentType : AbstractInventoryAnimal.DogEquipmentType.values()) {
            ...
            if (equipmentStack.getItem() instanceof DogEquipmentItem dogEquipmentItem) {
                ...
                if (dogEquipmentType != AbstractInventoryAnimal.DogEquipmentType.VEST || dogEquipmentItem instanceof DyeableDogEquipmentItem) {
                    ...
                    VertexConsumer vertexConsumer = buffer.getBuffer(RenderType.entityTranslucent(new ResourceLocation(WorkDog.MOD_ID, "textures/entity/jack_russell_terrier/equipment/jackrussell_" + typeLang + ".png")));
                    model.renderToBuffer(poseStack, vertexConsumer, packedLight, OverlayTexture.NO_OVERLAY, f, f1, f2, 1.0F);
                    VertexConsumer vertexConsumer2 = buffer.getBuffer(RenderType.entityCutoutNoCull(new ResourceLocation(WorkDog.MOD_ID, "textures/entity/jack_russell_terrier/equipment/jackrussell_" + typeLang + "_metal.png")));
                    model.renderToBuffer(poseStack, vertexConsumer2, packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);

                } else if (dogEquipmentItem instanceof ServiceVestItem serviceVestItem) {
                    VertexConsumer vertexConsumer = buffer.getBuffer(RenderType.entityTranslucent(new ResourceLocation(WorkDog.MOD_ID, "textures/entity/jack_russell_terrier/equipment/jackrussell_service_" + serviceVestItem.getColor().getName() + ".png")));
                    model.renderToBuffer(poseStack, vertexConsumer, packedLight, OverlayTexture.NO_OVERLAY, f, f1, f2, 1.0F);
                }
            }
        }
